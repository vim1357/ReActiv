...
- `AUTH_LOGIN_RATE_LIMIT_WINDOW_MS` (optional, default: `60000`; brute-force window for `/api/auth/login`)
- `AUTH_LOGIN_RATE_LIMIT_MAX_IP_ATTEMPTS` (optional, default: `40`; failed login attempts per IP within window)
- `AUTH_LOGIN_RATE_LIMIT_MAX_IP_LOGIN_ATTEMPTS` (optional, default: `12`; failed login attempts per IP+login within window)
- `ACTIVITY_RATE_LIMIT_WINDOW_MS` (optional, default: `60000`; rate-limit window for activity ingest)
- `ACTIVITY_RATE_LIMIT_MAX_EVENTS` (optional, default: `300`; authenticated activity events per user within window)
- `GUEST_ACTIVITY_RATE_LIMIT_MAX_EVENTS` (optional, default: `240`; guest activity events per session+IP key within window)
- `GUEST_ACTIVITY_RATE_LIMIT_MAX_EVENTS_PER_IP` (optional, default: `180`; guest activity events per IP within window)
