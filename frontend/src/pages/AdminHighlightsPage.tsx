...
const WEEKLY_HIGHLIGHTS: WeeklyHighlightItem[] = [
  {
    period: "22-28 февраля 2026",
    title: "Спринт 1. Запуск ядра платформы",
    points: [
      "Собран рабочий production-контур платформы и выведен первый продуктовый сценарий витрины.",
      "Запущены каталог, карточка, фильтрация и навигация.",
      "Сформирована базовая модель данных по лотам и импортам.",
      "Заложена архитектурная основа для подключения нескольких лизингодателей.",
    ],
  },
  {
    period: "1-7 марта 2026",
    title: "Спринт 2. Управляемость продукта и контроль данных",
    points: [
      "Запущен административный контур с ролями и доступами.",
      "Добавлена событийная аналитика и первый управленческий слой отчетности.",
      "Импорт переведен в контролируемый процесс с валидациями, аудитом партий и диагностикой ошибок.",
      "Введен контроль изменений между загрузками: прирост, выбытие и стабильная часть стока.",
    ],
  },
  {
    period: "8-14 марта 2026",
    title: "Спринт 3. Multi-source контур и качество карточек",
    points: [
      "Платформа переведена в multi-source режим с единой витриной для нескольких источников.",
      "Настроена нормализация типов техники, брендов и разнородных форматов данных.",
      "Реализовано массовое медиа-обогащение карточек по внешним идентификаторам.",
      "Поднят защищенный контур синхронизации и диагностики проблем по медиа-источникам.",
    ],
  },
  {
    period: "15-21 марта 2026",
    title: "Спринт 4. Масштабирование supply и управленческий слой",
    points: [
      "Подключены Альфа-Лизинг и Совкомбанк Лизинг, каталог масштабирован до ~17.8k позиций.",
      "Выведен устойчивый недельный прирост стока на уровне +3k позиций.",
      "Выполнен пакет performance- и UX-оптимизаций витрины.",
      "Добавлены пользовательские и управленческие функции: избранное, Highlights, KPI-слой и аналитика структуры стока.",
    ],
  },
];
...
export function AdminHighlightsPage() {
  ...
  const [mediaHealthError, setMediaHealthError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadSnapshot(): Promise<void> {
      ...
      try {
        ...
        const mediaHealthPromise = getAdminMediaHealth(MEDIA_HEALTH_HISTORY_DAYS)
          .then((value) => ({ ok: true as const, value }))
          .catch((caughtError) => ({ ok: false as const, caughtError }));

        const [
          baseMetricsResults,
          mediaHealthResult,
        ] = await Promise.all([baseMetricsPromise, mediaHealthPromise]);
        ...
        if (mediaHealthResult.ok) {
          setMediaHealthHistory(mediaHealthResult.value.history ?? []);
          setMediaHealthLatestRun(
            mediaHealthResult.value.recentRuns?.[0] ?? null,
          );
          setMediaHealthError(null);
        } else {
          const message =
            mediaHealthResult.caughtError instanceof Error
              ? mediaHealthResult.caughtError.message
              : "Не удалось загрузить блок живости медиа.";
          setMediaHealthHistory([]);
          setMediaHealthLatestRun(null);
          setMediaHealthError(message);
        }
      } catch (caughtError) {
        if (!isMounted) {
          return;
    ...
    <section className="highlights-page">
      ...
      <div className="panel highlights-media-health">
        ...
        {isLoading ? (
          ...
          <p className="error">{mediaHealthError}</p>
